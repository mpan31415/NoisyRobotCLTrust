...
bxp1 <- ggboxplot(
  data_df, x = "noisy_robot", y = "tlx_ave",
...
######################### check normality ######################### 
ggqqplot(data_df, "tlx_ave", ggtheme = theme_bw()) + facet_grid(autonomy ~ noisy_robot)

######################### check variance homogeneity ######################### 
data_df %>%
  group_by(autonomy) %>%
  levene_test(tlx_ave ~ noisy_robot)

######################### check covariance homogeneity ######################### 
box_m(data_df[, "tlx_ave", drop = FALSE], data_df$noisy_robot)
...
res.aov <- anova_test(data = data_df, dv = tlx_ave, wid = pid, between = noisy_robot, within = autonomy)
...
# pwc <- data_df %>%
#   group_by(autonomy) %>%
#   pairwise_t_test(tlx_ave ~ noisy_robot, p.adjust.method = "bonferroni")
# pwc
# 
