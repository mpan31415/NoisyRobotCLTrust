...
data_df <- read_csv("sliced_combined_data.csv", show_col_types = FALSE)
...
ggqqplot(data_df, "per_auto", ggtheme = theme_bw()) + facet_grid(auto_grouped ~ noisy_robot, labeller = "label_both")

######################### check variance homogeneity ######################### 
data_df %>%
  group_by(auto_grouped) %>%
  levene_test(per_auto ~ noisy_robot*order)
...
res.aov <- anova_test(data = data_df, dv = per_auto, wid = pid, within = auto_grouped, between = c(noisy_robot, order))
...
noisy.effect <- data_df %>%
  ...
  anova_test(dv = per_auto, wid = pid, within = auto_grouped)
noisy.effect
