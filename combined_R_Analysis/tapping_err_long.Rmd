...
data_df %>%
  group_by(noisy_robot, order, auto_grouped) %>%
  shapiro_test(tapping_err_long)

ggqqplot(data_df, "tapping_err_long", ggtheme = theme_bw()) + facet_grid(auto_grouped ~ noisy_robot, labeller = "label_both")

######################### check variance homogeneity ######################### 
data_df %>%
  group_by(auto_grouped) %>%
  levene_test(tapping_err_long ~ noisy_robot*order)
...
res.aov <- anova_test(data = data_df, dv = tapping_err_long, wid = pid, within = auto_grouped, between = c(noisy_robot, order))
